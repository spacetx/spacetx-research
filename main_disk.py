...
from MODULES.utilities_visualization import plot_overlap_and_fmaps, plot_img_and_seg, plot_concordance
...
new_index = torch.tensor([0,  4, 10,  12,  3,  5,  6,  7,  8,  9, 11, 2, 13, 14, 15, 16], dtype=torch.long)
reference_imgs, reference_seg, reference_count = test_loader.load(index=new_index)[:3]
            ...
            with torch.no_grad():
                ...
                if (epoch % TEST_FREQUENCY) == 0:
                    ...
                    output: Output = vae.forward(reference_imgs,
                                                 ...
                                                 draw_bg=True,
                                                 verbose=False)
                    fig_fmap = plot_overlap_and_fmaps(output, epoch=epoch, neptune_name="fmap_overlap")
