...
from .utilities import invert_convert_to_box_list, convert_to_box_list
from .utilities_visualization import plot_img_and_seg
...
class SimilarityKernel(torch.nn.Module):
    ...
    def __init__(self, n_kernels: int = 4,
                 pbc: bool = False,
                 eps: float = 1E-4,
                 length_scale: Tuple[float, float, float] = [2.0, 10.0, 50],
                 weight: Tuple[float, float, float] =  [0.1, 1.0, 10]):
        ...
        self.length_scale_min = min(length_scale)
        self.length_scale_max = max(length_scale)
        self.weight_min = min(weight)
        self.weight_max = max(weight)

        self.l = torch.nn.Parameter(data=torch.tensor(length_scale[1], device=self.device, dtype=torch.float),
                                    requires_grad=True)
        self.w =  torch.nn.Parameter(data=torch.tensor(weight[1], device=self.device, dtype=torch.float),
                                    requires_grad=True)
    ...
    def get_l_w(self):
        ...
        l_clamped = self.l.clamp_(min=self.length_scale_min, max=self.length_scale_max)
        w_clamped = self.w.clamp_(min=self.weight_min, max=self.weight_max)
    ...
    def forward(self, n_width: int, n_height: int):
        ...
        print("DEBUG self.d2.shape ->", self.d2.shape)
        likelihood_kernel = self.diag + w*torch.exp(-0.5*self.d2/(l*l))
